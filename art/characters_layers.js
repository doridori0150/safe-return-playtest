...
(function (root) {
  'use strict';
  // 종이 인형용 포크: o.layer = 'head' | 'torso' | 'item' 이면 그 층만 그린다 (원본 characters.js는 그대로 둔다)
  let LAYER=null;const L=n=>!LAYER||LAYER===n;
  ...
  function jewelry(c,f,back) {
    ...
    if(f.ribbon&&L('torso')) {const x=sx(f.ribbon.side,58,141),y=184; s+=feature('ribbon',p(`M${x} ${y} l-13 -7 l-2 14 l14 -3 l-6 14 l8 -4 l5 -12 l10 5 l1 -15Z`,f.ribbon.color,1.8)+e(x,y+2,3,3,f.ribbon.color,1.3)+p(`M${x-9} ${y} l6 2 M${x+5} ${y+1} l6 -2`,'none',1.2));}
    if(!back && f.circlet && L('head')) s+=feature('circlet',p('M65 88 Q99 96 136 88','none',3,'#dedce8')+p('M93 92 L100 85 L107 92 L100 101Z','#e9e8ed',1.3)+p('M97 92 L100 89 L103 92 L100 97Z','#8a77ac',.8));
    if(!back && f.necklace && L('torso')) {s+=feature('necklace',p('M74 156 Q76 190 122 179 L131 157','none',2,'#73553c')+(f.necklace==='bone'?[82,97,113].map((x,i)=>g(p('M-3 -8 Q-8 -12 -9 -6 Q-9 -2 -4 -2 L-4 7 Q-9 10 -5 13 Q-1 15 1 10 Q7 13 8 8 Q8 4 3 5 L3 -3 Q8 -5 5 -9 Q2 -11 -3 -8Z',BONE,1.4),`translate(${x} ${i===1?182:178}) rotate(${i===0?-22:20})`)).join(''):e(100,179,7,8,GOLD,1.7)));}
    return s;
  }
  function beard(c,f) {
  ...
  function overlays(c,o,back) {
    let s='';
    if(o.mushroom)s+=feature('mushroom',(L('head')?mushroom(79,61,10)+mushroom(117,55,13):'')+(back||!L('torso')?'':mushroom(143,184,8)));
    if(o.moss)s+=feature('moss',(L('torso')?moss(50,200,.8)+moss(145,202,.65):'')+(back||!L('head')?'':moss(63,98,.35)));
    if(back&&o.backMush&&L('torso'))s+=feature('backMush',mushroom(77,192,12)+mushroom(122,188,15)+mushroom(110,222,9));
    if(back&&o.backMoss&&L('torso'))s+=feature('backMoss',moss(91,199,1)+moss(122,219,.7));
    if(!back&&o.bandage&&L('torso')){let b=p('M31 198 L64 204 L61 222 L28 215Z','#dfd9c5',1.8)+p('M31 203 l32 5 M30 209 l31 5','none',1.2,'#ada68f');s+=feature('bandage',c.side==='R'?b:flip(b));}
    if(!back&&o.mud&&L('torso'))s+=feature('mud',p('M48 222 q-8 -9 -10 1 l3 11 l17 5 l8 -7 l-5 -10Z M124 228 q-8 -11 -13 -1 l-1 13 h26 l5 -8Z','#66503c',0)+e(72,222,3,4,'#66503c')+e(139,219,4,2,'#66503c'));
    return s;
  }
  function backFace(o) {
  ...
  function fig(id,o={},back=false) {
    ...
    let s='';LAYER=o.layer||null;
    if(id==='hood'||id==='void'){s=LAYER&&LAYER!=='head'?'':(id==='hood'?hood(c,f,o,back):voidFigure(c,o,back));}
    else {
      s=(L('torso')?body(c,back):'')+(back||!L('head')?'':hairBack(c,f,false))+(L('head')?neck(c,f,o,back):'');
      if(L('head')){
      ...
      s+=jewelry(c,f,back);
      if(back&&o.backFace&&L('head'))s+=backFace(o);
      s+=(L('item')?item(c,back):'')+overlays(c,o,back);
    }
    LAYER=null;
    if(back)s=flip(s);
  ...
  const api={fig,shadowExtra,DATA,layered:true};
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root)root.CHAR_ART_L=api;
